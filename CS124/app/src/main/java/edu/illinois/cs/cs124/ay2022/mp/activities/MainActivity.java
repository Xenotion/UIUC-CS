...
public final class MainActivity extends AppCompatActivity
  ...
  protected void onCreate(final Bundle unused) {
    ...
    IGeoPoint center = new GeoPoint(40.10986682167534, -88.22831928981661);
  ...
  public boolean onQueryTextChange(final String text) {
    List<Place> arr = Place.search(allPlaces, text);
    if (arr.isEmpty()) {
      updateShownPlaces(allPlaces);
    } else {
      updateShownPlaces(arr);
    }
    return true;
  }
  ...
  public boolean longPressHelper(final GeoPoint p) {
    ...
    launchAddFavoritePlace.putExtra("latitude", Double.toString(p.getLatitude()));
    launchAddFavoritePlace.putExtra("longitude", Double.toString(p.getLongitude()));
